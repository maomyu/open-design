...
import { flushEmbeddedBrowserCookieVault, hardenWebviewEmbeddedBrowser, registerEmbeddedBrowserBridge, registerEmbeddedBrowserCookieBridge, registerEmbeddedBrowserLoginProbeBridge, registerWebviewFileInputBridge, restoreEmbeddedBrowserCookies } from "./embedded-browser.js";
...
export async function createDesktopRuntime(options: DesktopRuntimeOptions): Promise<DesktopRuntime> {
  ...
  registerEmbeddedBrowserLoginProbeBridge();
  ...
  const requireMainWindowSender = (event: Electron.IpcMainInvokeEvent): void => {
    if (event.sender !== window.webContents) {
      throw new Error("updater IPC is only available to the main social-auto window");
    }
  };
