...
// 视频号登录:只清 UA(session + webContents),与煜见分支对齐(2026-07-21 用户报登录二维码
// 「加载失败」/登不进去)。此前加过的 Client Hints(sec-ch-ua 补 Google Chrome 品牌 + JS 层
// 覆盖 navigator.userAgentData)和 mmfinderassistant 的 Referer 改写全部移除——实测这两块会
// 干扰微信登录页的二维码/auth 请求(注入 JS 在登录 iframe 里跑、改请求头)导致二维码加载失败;
// 煜见分支只做 cleanUserAgent 就能扫码登录。cleanUserAgent 去掉 Electron/WorkBuild 标记即可
// (微信登录页读 navigator.userAgent 检测内置浏览器,session UA + webContents UA 双清)。
