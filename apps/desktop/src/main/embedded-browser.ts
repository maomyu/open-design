...
function cleanUserAgent(): string {
  return app.userAgentFallback
    .split(" ")
    .filter((token) => !/^(Electron|open-design|OpenDesign|WorkBuild|WorkBuild)\//i.test(token))
    .join(" ");
}
...
export function registerEmbeddedBrowserCookieBridge(): void {
  ...
  ipcMain.handle(EXPORT_COOKIES_IPC_CHANNEL, async (_event, raw: unknown) => {
    ...
    try {
      ...
      const lines = ["# Netscape HTTP Cookie File", "# generated by WorkBuild embedded browser", ""];
      for (const c of cookies) {
