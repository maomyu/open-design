// 百度知道创作台 — 独立一级入口(2026-07-10 用户拍板:按平台性质分台,不融进
// 公众号;同构架构:选题 → 写作 → 发布)。
//
...
const PLATFORM = 'baidu-zhidao';
const LAST_ARTICLE_KEY = 'open-design:studio:last-baidu-zhidao-article';
...
export function BaiduZhidaoStudioView(): JSX.Element {
  ...
  return (
    <div className={c('root')}>
      <div className={c('head')}>
        <h1 className={c('title')}>百度知道创作台</h1>
      ...
      <div className={c('tabs')} role="tablist" aria-label="百度知道创作台导航">
        {TABS.map((item) => (
          <button
            key={item.id}
            type="button"
              ...
              <>
                ...
                <div className={c('card')}>
                  ...
                  {hasFeature(license, 'cap.ai') ? (
                    <>
                      <div className={c('row')}>
                        <select className={c('select')} value={aiWordCount} onChange={(e) => { setAiWordCount(e.target.value); saveStudioPref('wordcount:baidu-zhidao', e.target.value, '1500-2000'); }}>
            ...
            !article ? (
              ...
              <>
                <SafeHandoffCard
                  ...
                  targets={[{ id: 'baidu-zhidao', label: '百度知道' }]}
                  defaultTarget="baidu-zhidao"
                  hasAssets={false}
