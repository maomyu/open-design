          ...
          <button
            ...
            onClick={() => {
              if (!state.company.trim()) {
                setError('Tell WorkBuild about the company or design system first.');
                return;
              }
              setStep('confirm');
            }}
  ...
  const sendProjectChatMessage = useCallback(
    async (
      ...
      if (config.mode !== 'daemon' || !config.agentId) {
        setChatError('Pick a local agent first, then ask WorkBuild to update this design system.');
        return;
      }
          ...
          <div className="ds-review-column">
              ...
              <p>
                {generationActive
                  ? activeJob?.kind === 'revision'
                    ? 'WorkBuild is applying your feedback. You can keep reviewing while the updated draft is prepared.'
                    : 'WorkBuild is still working, but you can start giving feedback on the work so far.'
                  : 'WorkBuild is ready for review. Give feedback on the work so far, then publish when it is useful for future projects.'}
              </p>
              <label>
                <input
                  type="checkbox"
            ...
            <div className="ds-warning-card">
              ...
              <span>
                ...
                WorkBuild is rendering typography with substitute web fonts.
              </span>
    ...
    <section className={`ds-workspace-activity-card is-${status}`}>
      <div className="ds-workspace-activity-head">
        ...
        <span>
          <strong>
            {status === 'running'
              ? 'WorkBuild is updating this system'
              : status === 'failed'
      ...
      <div>
        ...
        <span>
          <strong>
            {active
              ? job.kind === 'revision'
                ? 'WorkBuild is revising'
                : 'WorkBuild is still working'
              : job.status === 'failed'
...
function GitHubRepositoryAccessPanel({
  ...
  const methods: GitHubAccessMethod[] = [
    ...
    },
    {
      id: 'native-oauth',
      icon: 'link',
      title: 'WorkBuild account',
      badge: 'Coming soon',
      tone: 'muted',
      description: 'Native GitHub sign-in managed by WorkBuild; this build does not use an OD-managed GitHub token yet.',
    },
    {
      id: 'composio',
      icon: composioIcon,
    ...
    >
      <div className="ds-github-access-header">
        <span>
          ...
          <p>Paste a GitHub URL. WorkBuild will use the first working access method.</p>
        </span>
        <button
          type="button"
...
function buildCreationAgentPrompt(
  ...
  return [
    ...
    state.codeFolders.length
      ? `Read the linked local code folders that WorkBuild attached to this project: ${state.codeFolders.join(', ')}. Treat them as source context only unless the user asks you to edit them.\n\n${localFolderRunbook}`
      : '',
...
function buildSourceContextManifest(
  ...
  sections.push(
    ...
    '- README.md and SKILL.md should make the extracted system reusable as a real WorkBuild design-system package.',
