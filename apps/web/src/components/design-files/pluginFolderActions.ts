...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the social-auto community catalog at `nexu-io/open-design`.',
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic social-auto CLI workflow from the current project workspace:',
    '',
...
// social-auto issue URL and belongs to the "social-auto PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits a social-auto issue URL and belongs to the "social-auto PR" button.',
    '',
    'Run this deterministic social-auto CLI workflow from the current project workspace:',
    '',
