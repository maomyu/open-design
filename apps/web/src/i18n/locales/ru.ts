...
export const ru: Dict = {
  ...
  'chat.amrCard.switchBody': 'Переключитесь на официальный сервис моделей AMR от WorkBuild — без настройки API-ключа. После входа, авторизации и пополнения это выполнение будет автоматически повторено.',
  ...
  'creator.eyebrow': "Создать",
  'creator.pluginTitle': "Создать плагин",
  'creator.skillTitle': "Создать навык",
  'creator.pluginLede': "Опишите рабочий процесс своими словами — что он должен делать, примерные шаги и где вы хотите подтверждать перед продолжением. ИИ подготовит черновик полного плагина (методология, стартовый бриф, шаги workflow) для вашей проверки; ничего не сохраняется без вашего подтверждения.",
  'creator.skillLede': "Опишите, что должен делать навык и когда его использовать. ИИ подготовит полный SKILL.md — срабатывающее описание, методологию и несколько тестовых prompt — для вашей проверки; ничего не сохраняется без вашего подтверждения.",
  'creator.pluginPlaceholder': "напр. Еженедельный конвейер коротких видео для моего родительского аккаунта: выбрать трендовые темы (я подтверждаю), написать сценарий (я подтверждаю), затем сгенерировать видео и завершить итогами.",
  'creator.skillPlaceholder': "напр. Писать 3-секундные хуки для начала коротких видео. Использовать всякий раз, когда я упоминаю хуки, начало или удержание. Выдавать 5 вариантов, отсортированных по силе.",
  'creator.targetPlugin': "Плагин",
  'creator.targetSkill': "Навык",
  'creator.draftButton': "Черновик с ИИ",
  'creator.drafting': "Создание черновика…",
  'creator.redraft': "Создать черновик заново",
  'creator.draftFailed': "Не удалось создать черновик. Проверьте настройки модели и попробуйте снова.",
  'creator.save': "Сохранить и опубликовать",
  'creator.saving': "Сохранение…",
  'creator.saveFailed': "Не удалось сохранить.",
  'creator.skillSaved': "Навык сохранён в вашей библиотеке.",
  'creator.nameLabel': "ID",
  'creator.titleLabel': "Название",
  'creator.descriptionLabel': "Описание",
  'creator.stagesHint': "Пошаговые prompt с точками подтверждения. После сохранения донастройте их в полном редакторе.",
  'creator.triggersLabel': "Триггеры (через запятую)",
  'creator.bodyLabel': "Текст SKILL.md",
  'creator.evalsLabel': "Тестовые prompt",
  'creator.evalsHint': "Сохраняется как evals/evals.json — позже используется для тестирования и улучшения навыка.",
  ...
  'settings.agentInstall.pathHint':
    'Если вы установили CLI через npm или Homebrew, но она всё ещё отмечена как не установленная, убедитесь, что bin-каталог инструмента есть в PATH, который наследует daemon WorkBuild (в macOS PATH у Terminal и GUI-приложений может отличаться). См. QUICKSTART.md (раздел "Local agent CLI and PATH").',
  'settings.agentInstall.stepOpenLinks': 'Откройте Установить или Документация для нужного агента.',
  'settings.agentInstall.stepAuth':
    'Пройдите аутентификацию в CLI поставщика (вход или добавление API-ключей), затем вернитесь в WorkBuild.',
  ...
  'settings.mcpServerHint': 'Откройте WorkBuild как MCP-сервер для вашего кодинг-агента.',
  ...
  'entry.githubStarAria': 'Поставить WorkBuild звезду на GitHub',
  ...
  'preview.errorBody': 'Не удалось получить HTML примера. Убедитесь, что WorkBuild запущен, и повторите попытку.',
  ...
  'pet.tabBuiltInHint': 'Подобранные питомцы в комплекте WorkBuild — выберите и забирайте.',
  ...
  'pet.codexBundledTitle': 'Поставляется с WorkBuild — загрузка не нужна.',
  ...
  'updater.availableBody': 'WorkBuild {version} доступен. Он будет загружен перед открытием установщика.',
  ...
  'updater.installerOpenBody': 'Установщик открыт. WorkBuild закрывается, чтобы вы могли завершить обновление.',
  ...
  'updater.quitButton': 'Закрыть WorkBuild',
  'updater.quitFailedBody': 'Установщик открыт, но WorkBuild не удалось закрыть. Закройте WorkBuild перед заменой приложения.',
  'updater.quitFailedTitle': 'Не удалось закрыть',
  'updater.quitting': 'Закрытие...',
  'updater.ready': 'Обновление готово',
  'updater.readyGeneric': 'Новая версия готова. WorkBuild закроется и откроет установщик.',
  'updater.readyVersion': 'WorkBuild {version} готов. WorkBuild закроется и откроет установщик.',
