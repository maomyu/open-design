...
/**
 * 平台风控/验证特征文案(全平台通用一份):命中即判【触发风控】,互动执行立刻停,
 * 绝不继续外发(风控下继续猛发只会加速封号)。词都取「验证/限流」的强特征,避开
 * 笔记正文可能出现的普通词——正文写「验证码」三个字不算,必须是「安全验证/拖动滑块/
 * 操作频繁/稍后再试」这类平台提示语。
 */
export const RISK_CONTROL_MARKERS: readonly string[] = [
  '安全验证',
  '拖动滑块',
  '滑块验证',
  '滑动验证',
