...
async function runBaokuan(args) {
  ...
  try {
    flags = parseFlags(rest, {
      string: ['keyword', 'platforms', 'pages', 'daemon-url', 'daemon-port',
               'title', 'subtitle', 'ref', 'style-json', 'versions', 'bg', 'out', 'out-dir', 'platform',
               'days', 'limit'],
      boolean: ['json'],
    });
  } catch (err) {
  ...
  switch (sub) {
    ...
    default:
      console.error(`未知子命令: ${sub}（collect|cover|cost|failed|retry|backup|autostart）`);
      process.exit(2);
  }
}
