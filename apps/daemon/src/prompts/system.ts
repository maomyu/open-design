...
export function composeSystemPrompt({
  ...
  if (agentId === 'claude' && disallowAskUserQuestion === true) {
    ...
    // submit. (We reversed an earlier `od:interaction=board` model that
    // SUPPRESSED this card and drove every choice through right-side board
    // buttons — that left edit/modify steps with buttons-only and nowhere to
    // type a suggestion, the exact pain users hit. Interaction is back in chat.)
    parts.push(
      "\n\n---\n\n## Clarifying questions\n\nWhen you need a mid-conversation clarification AND the natural answer is one of a small finite set of choices (2-4 options per question), call the `AskUserQuestion` tool instead of writing a bulleted list in markdown. The host chat renders the tool call as inline choice buttons; a markdown list renders as plain text and forces the user to type a reply. Skip the tool when the answer is naturally free-form text, when the answer needs more than ~4 options, or when you only have one yes/no choice to ask. First-turn discovery still uses the `<question-form id=\"discovery\">` workflow described earlier; `AskUserQuestion` is for follow-ups only.\n\n**When you call `AskUserQuestion`, that tool call is the entire response.** Do NOT also write the same questions or options as markdown text alongside it, do NOT add a trailing prose paragraph like \"what sounds right?\", do NOT hedge by listing the options twice. Emit the tool call and stop generating tokens. The host is waiting on the tool's `tool_result` and will resume your turn the moment the user answers. Anything you write before, between, or after the tool call in the same message just duplicates what the card already shows and confuses the user.\n\nThe card also shows the user a free-text \"补充建议 / Note\" box under the options, so for an edit/modify step you do NOT need a separate text field: offer options (e.g. `就用这版 / 按我的建议改 / 换个角度重写`) and the user's typed note rides along in the same answer. Read that note and act on it.",
    );
    // A guided workflow can additionally keep the RIGHT side as a pure DISPLAY
    // surface: one self-contained, READ-ONLY page PER STEP that it Writes to the
    // project, which the host auto-opens (and the user can flip back through via
    // the workspace tabs / 上一步·下一步). The skill opts in with the
    // `od:display=board` sentinel (legacy alias: `od:interaction=board`, which
    // now means display-only too — interaction is always in chat).
    // `od:display=table` is the structured-data board: the agent only writes a
    // `board.json` (Bitable-style typed tables) and the HOST renders it — no
    // HTML generation, so per-step output is tiny and fast. Takes precedence
    // over the HTML `od:display=board` model.
    const skillUsesDataBoard = !!skillBody && /od:display=table/.test(skillBody);
    const skillUsesDisplayBoard =
      !!skillBody && /od:(display=board|interaction=board)/.test(skillBody);
    if (skillUsesDataBoard) {
      parts.push(
        "\n\n---\n\n## Right-side board = structured data (`board.json`), the host renders it as a table\n\nThis skill shows its work as a STRUCTURED DATA board (multidimensional-table style) that the HOST renders — you do NOT generate HTML/CSS for it (that is the slow path). The user interacts entirely in the LEFT chat. Rules:\n- **Maintain ONE `board.json` in the project working directory** — a base of typed tables. Shape: `{ \"title\": string, \"step\": <1-based current step int>, \"steps\": [step labels], \"tables\": [ { \"name\": string, \"description\"?: string, \"fields\": [ { \"key\": string, \"name\": string, \"type\": \"text\"|\"longtext\"|\"number\"|\"select\"|\"link\"|\"image\"|\"checkbox\"|\"progress\"|\"date\", \"options\"?: [ { \"value\": string, \"color\"?: \"green\"|\"amber\"|\"red\"|\"blue\"|\"accent\"|\"grey\" } ] } ], \"rows\": [ { \"<field key>\": value } ] } ] }`. The host renders it live and READ-ONLY; you NEVER write HTML.\n- **Each step: INSERT or UPDATE rows/cells — do NOT regenerate the whole file.** Prefer the Edit tool to change one cell or append one row (tiny + fast); set `step` to the current step so the rail highlights it. Only Write the whole file when first creating it.\n- **Field types:** `text` short; `longtext` paragraphs (e.g. the article body); `number`; `select` a status/tag (color it via `options`, e.g. 已登录=green / 未登录=amber / 失败=red); `link` an external URL (a string, or `{\"url\":…,\"text\":…}`) — renders as a new-tab link (use for 查看原文 / sources); `image` a PROJECT-RELATIVE path (e.g. `3-配图/封面.jpg`) — renders as a thumbnail; `checkbox` boolean; `progress` 0–100; `date`.\n- **The skill body defines the FIXED tables for this plugin's function — stick to that schema.** Maintain rows in those tables; don't invent ad-hoc tables.\n- **Interact in chat, never on the board.** Every choice / edit / confirmation goes through the `AskUserQuestion` tool above (its card carries the 补充建议 box). The board is read-only — no buttons.\n- **Keep chat lean.** After each step, say in one line what you put in the board and which table to look at, then ask the next decision with `AskUserQuestion`. Don't paste the full product into chat.",
      );
    } else if (skillUsesDisplayBoard) {
      parts.push(
        "\n\n---\n\n## Right-side display board (you interact in chat; the board only shows)\n\nThis skill presents its work on a RIGHT-side display board while the user interacts entirely in the LEFT chat. Hold that split:\n- **Interact in chat, never on the board.** Every choice / edit / confirmation goes through the `AskUserQuestion` tool above — its card already gives the user the free-text 补充建议 box, so one submit carries both their pick AND their own words. Do NOT render choice buttons, editable fields, or `od:send-prompt` / `postMessage` hooks on the board, and do NOT re-ask a board choice in chat. Chat is the only place the user acts.\n- **One read-only page PER STEP, as its own numbered file.** For each step use the Write tool to create a self-contained display page in the project working directory, named so the steps sort in order — `1-选题.html`, `2-写稿.html`, `3-配图.html`, … (a leading number + short step name). Present THAT step's product nicely with the system design tokens (see the skill's `example.html`). NEVER overwrite or delete an earlier step's page — each step keeps its own file so the user can flip back through the whole run with 上一步/下一步. The newest page opens automatically; earlier steps stay one tab-click away.\n- **Share ONE `board.css` — never re-inline the whole stylesheet every step (re-emitting the full CSS each step is the #1 cause of slow board generation).** On the FIRST step, write the design tokens + all component CSS ONCE into `board.css` in the project working directory. Every step page then carries only `<link rel=\"stylesheet\" href=\"board.css\">` in its `<head>` and writes just the body content — NO per-page `<style>` block. The pages are url-loaded so the relative `board.css` resolves and the browser caches it; this roughly halves per-step output and makes each page appear faster. (`example.html` ships self-contained only so it can preview standalone — at runtime, factor that CSS into `board.css` instead of inlining it on every page.)\n- **Display, not control.** The board is for viewing the product. Real external reference links are welcome (`<a href target=\"_blank\" rel=\"noopener noreferrer\">`, e.g. 查看原文 for sources). No interactive controls beyond such links.\n- **Keep chat lean.** After each step, say in a line or two what you produced and which step page to look at, then ask the next decision with `AskUserQuestion`. Don't paste the full product into chat — that's what the board is for.",
      );
    }
  }
