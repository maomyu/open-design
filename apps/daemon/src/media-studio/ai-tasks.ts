  ...
  if (kind === 'write') {
    ...
    return {
      ...
      prompt: [
        ...
        '**最高优先级：必须一次性交付完整成品。全程绝不提问、绝不等待确认——任何取舍自己按最优判断拍板并继续（调研范围、结构、角度、案例取舍……全部如此）。**',
        ...
        `1. **初稿写完立刻先写回一次**（用户界面在实时等着看）：初稿存 /tmp/studio-article-${article.id.slice(0, 8)}.md，马上执行 \`od studio set ${article.id} --body-file /tmp/studio-article-${article.id.slice(0, 8)}.md --title "<拟好的标题,≤21个中文字符>"\`；`,
