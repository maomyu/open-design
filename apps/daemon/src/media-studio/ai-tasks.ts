  ...
  if (kind === 'write') {
    ...
    return {
      ...
      prompt: [
        ...
        '- 在正文里标注配图位：`<!-- IMAGE_N: 画面描述, 4:3 -->`（封面 `<!-- IMAGE_COVER: 描述, 16:9 -->` 放最前）。**标注必须紧跟在它要配图的那个段落后面**（生图会按这个位置取上文段落做场景依据）。描述只写画面内容,**禁止包含画风词**(插画/手绘/摄影/3D等)——画风由所选统一风格在生图时动态注入。',
        ...
        '- **配图描述是直接发给生图模型的完整提示词，必须细致**，40~120 字，且画的必须是所在段落讲的事。按分镜要素写全：主体（谁/什么，外观特征）+ 动作或状态 + 环境与关键道具（来自该段的具体事物）+ 构图与视角（特写/俯视/中景…）+ 光线与色调 + 质感/情绪。禁止「商务场景」「示意图」「相关配图」这类空泛词；画面里不要出现可读文字（生图模型写不好字）；讲数据/政策的段落用可视化隐喻（上涨曲线、天平、盖章的文件、放大镜看合同等）。示例：段落讲「社保断缴影响购房资格」→ `<!-- IMAGE_2: 特写镜头，一只手正把社保卡插回桌上的读卡器，卡面微微反光；旁边摊开的购房合同压着一串黄铜钥匙，背景是虚化的窗光客厅；暖色台灯光从左侧打来，氛围略带紧迫, 4:3 -->`。',
