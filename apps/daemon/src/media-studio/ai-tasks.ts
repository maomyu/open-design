  ...
  if (kind === 'write') {
    ...
    return {
      ...
      prompt: [
        ...
        '- 按写作方法论在正文里标注配图位：`<!-- IMAGE_N: 画面描述, 4:3 -->`（封面 `<!-- IMAGE_COVER: 描述, 16:9 -->` 放最前）。**标注必须紧跟在它要配图的那个段落后面**（生图会按这个位置取上文段落做场景依据）。描述只写画面内容,**禁止包含画风词**(插画/手绘/摄影/3D等)——画风由所选统一风格在生图时动态注入。',
        '- **配图描述要像给画师的分镜脚本，且必须画的是所在段落讲的事**：把该段核心内容画面化——主体（谁/什么）+ 动作或状态 + 环境与关键道具 + 氛围/光线，20~50 字。禁止「商务场景」「示意图」「相关配图」这类空泛词；画面里不要出现可读文字（生图模型写不好字）；讲数据/政策的段落用可视化隐喻（上涨曲线、天平、盖章的文件、放大镜看合同等）。示例：段落讲「社保断缴影响购房资格」→ `<!-- IMAGE_2: 一只手把社保卡插回读卡器，旁边摊着购房合同和钥匙，桌面暖光, 4:3 -->`。',
