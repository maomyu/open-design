...
import { styleAllowsText } from './qwen-image.js';
  ...
  if (kind === 'write') {
    ...
    return {
      ...
      prompt: [
        ...
        `- **配图描述是直接发给生图模型的完整提示词，必须细致**，40~120 字，且画的必须是所在段落讲的事。按分镜要素写全：主体（谁/什么，外观特征）+ 动作或状态 + 环境与关键道具（来自该段的具体事物）+ 构图与视角（特写/俯视/中景…）+ 光线与色调 + 质感/情绪。禁止「商务场景」「示意图」「相关配图」这类空泛词；${
          input.imageStyle?.trim() && styleAllowsText(input.imageStyle.trim())
            ? '该风格画面可以带文字元素（手写短词/标签/图解标注等，单处别超一句话，长句生图模型写不好）——**严禁在描述里写「不含文字/无文字/不要出现文字」这类禁字句**，那会顶掉该风格的手写/标签精髓'
            : '画面里不要出现可读文字（生图模型写不好字）'
        }；讲数据/政策的段落用可视化隐喻（上涨曲线、天平、盖章的文件、放大镜看合同等）。示例：段落讲「社保断缴影响购房资格」→ \`<!-- IMAGE_2: 特写镜头，一只手正把社保卡插回桌上的读卡器，卡面微微反光；旁边摊开的购房合同压着一串黄铜钥匙，背景是虚化的窗光客厅；暖色台灯光从左侧打来，写实摄影质感，氛围略带紧迫, 4:3 -->\`（示例按摄影风格写；实际质感/媒介用词跟随所选配图风格）。`,
