...
writeFileSync(
  ...
  JSON.stringify(
    {
      name: 'social-auto-cli',
      version: daemonPkg.version,
      description:
        'social-auto 媒体创作台命令行(选题/AI 写作/配图/排版/发布全流水线;供智能体与终端使用,需 social-auto 桌面端或 daemon 在本机运行)',
      license: 'Apache-2.0',
      type: 'module',
      bin: { social-auto: './cli.mjs', od: './cli.mjs' },
      engines: { node: '>=18.17' },
...
前提:本机安装并运行 social-auto 桌面端(CLI 自动发现其后台服务,零配置)。

## 快速验证

\`\`\`bash
social-auto studio --help          # 全部命令(od 是等价别名)
social-auto studio articles        # 公众号文章列表
\`\`\`

## 常用

\`\`\`bash
social-auto studio ai write <文章id> --words 1500-2000   # AI 写稿(等待完成)
social-auto studio publish <文章id> --account <账号id>   # 公众号 → 草稿箱
social-auto studio handoff <文章id> --target weibo --auto # 微博直发(需桌面端)
\`\`\`
...
const tarball = path.join(outRoot, `social-auto-cli-${daemonPkg.version}.tgz`);
