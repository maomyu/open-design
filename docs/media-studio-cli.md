...
`handoff` 是唯一依赖 **WorkBuild 桌面应用在运行**的命令：注入引擎跑在桌面端内嵌浏览器里（登录态在桌面端分区）。桌面端没开时 create 立即失败（exit 4，报错说清楚开桌面应用再试）。进度逐行打到 stdout，终态即退出码。`--account` 不给时用账号中心该平台第一个绑定账号。
...
| 现象 | 原因/处理 |
|---|---|
| exit 4「桌面端未连接」 | 打开 WorkBuild 桌面应用后重试 |
