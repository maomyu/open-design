...
2. **AI 定"爆款标准"（关键一步，别用死阈值）**：
   - **⭐先看有没有界面传来的 `[爆款筛选]` 块**：平台「选题」界面的爆款筛选控件（时间窗下拉 + 低粉/高播放/高赞 chip）会把用户勾选拼进消息，形如
     `[爆款筛选] 时间窗=近半年(180d)；命中任一规则：低粉爆款(粉丝≤3000·播放≥10万) 或 高播放…。请用「爆款雷达」技能按 --time-window 180d --criteria '{"time_window":"180d","rules":[...]}' 采集+评分。`
     **有这个块就直接照它给的 `--time-window` 和 `--criteria` 跑，不用再问用户**（他已经在界面上勾好了）。
   - 没有该块时，再用 **AskUserQuestion** 让用户确认，把口语转成结构化标准：
