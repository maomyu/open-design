...
### Phase A0 · 细节交互打磨层（2026-07-07 追加，当日全量落地 ✅）

用户拍板：功能骨架已齐，下一轮优先打磨「每个步骤的细节交互」。分两层：

**通用交互底座（一次做，三台全收益）**
1. 自动保存 + 状态徽标：编辑即防抖落库，头部常显「已保存 · 刚刚 / 保存中…」，
   消灭丢稿焦虑（曾真实发生：daemon 重启杀掉在途写作）。
2. 统一 toast 反馈体系：成功/失败/进行中三态，替代 alert 与零反馈按钮。
3. 步骤智能接力：每步完成后出现「下一步 →」主按钮（采纳选题→开写、
   写完→封面、封面完→发布），步骤条完成态可点回。
4. AI 任务全局状态条：跨 tab 常显「AI 正在写作…（查看/取消）」，任务结束
   toast 通知——彻底解决「AI 写没写、写到哪了」的感知问题。
5. 空状态引导：无文章/无选题/未配账号时一句话说明 + 主按钮，不留空白。
6. 键盘：Cmd+S 保存、Cmd+Enter 触发当前步主 AI 动作。

**各台逐步打磨（按使用频率排序）**
- 公众号·选题：候选卡「开写」一键建文章并带入标题+来源；来源文章
  「看原文」走内置浏览器。写作：手机宽度(375px)预览切换；IMAGE 标记行内
  高亮点击跳配图；划选段落→「按我说的改」只改选区。封面：一次出多张
  候选对比选用。发布：成功页「去公众号后台看草稿」内置浏览器直达。
- 短视频·脚本：钩子/正文/结尾三段结构化编辑 + 分段时长小计。配音：多音色
  试听（短句 30 字）。成片：「选择本机视频」文件选择器替代手填路径（桌面壳
  能力）。发布：「一键检查全部平台登录」；定时给「今晚8点/明早7点」快捷项。
- 笔记·图集：拖拽排序替代←→；批量生成逐张进度(3/6)；大图 lightbox 预览；
  单张重生成回填原提示词。文案：AI 标签一键加入 chips。发布：安全发布
  四步完成态持久化；分段复制（标题/正文/标签各一键，即原 C8 提前）。

### Phase A · 产能跃迁（细节层之后或并行）
