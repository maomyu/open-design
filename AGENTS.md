...
- `RuntimeAgentDef.promptInputFormat` selects how the daemon writes the prompt to a child's stdin. The default `'text'` writes the composed prompt and ends stdin immediately. `'stream-json'` wraps the prompt as one JSONL `user` message and KEEPS stdin open so the daemon can stream further user messages back in mid-turn. Claude (`apps/daemon/src/runtimes/defs/claude.ts`) ships `'stream-json'` together with `--input-format stream-json`. NOTE: this does NOT let the host answer `AskUserQuestion` via a stdin `tool_result` — claude-code in headless `-p` mode auto-denies AskUserQuestion and treats a stdin tool_result as a permission denial (confirmed against the CLI + docs; only the Agent SDK `canUseTool` callback can host-answer it, which `-p` does not use). AskUserQuestion answers are instead sent as a NORMAL user message (a new turn) — see the AskUserQuestion bullet below. The stream-json stdin channel stays for any future genuinely host-answerable tool. Every other agent stays on `'text'`.
- `apps/daemon/src/server.ts` tracks `run.pendingHostAnswers` (a Set of `tool_use_id` strings) and `run.stdinOpen` on the run object. The `claude-stream-json` event handler records AskUserQuestion ids in `run.askUserQuestionIds` (used ONLY to suppress claude-code's canned placeholder `tool_result` for them, e.g. "Answer questions?") but does NOT add them to `pendingHostAnswers` — we do not keep stdin open for AskUserQuestion (it cannot be host-answered), so the run ends normally and the conversation is free to receive the user's answer as a new message. stdin closes when `pendingHostAnswers` is empty AND a `turn_end` (or `usage`) event arrives with a non `tool_use` `stop_reason`. The `tool_use` stop reason means the model paused mid tool (waiting on claude-code's internal runner or on a host answer); closing stdin there would truncate the follow up response.
...
- AskUserQuestion specifically: it is NOT host-answerable via a stdin `tool_result` in `-p` mode (claude-code auto-denies it). The card submits the answer as a NORMAL user message (`ToolCard.handleSubmit` → `onSubmitForm` → `ProjectView.handleSend` → new run), which the model reliably reads and which records the answer in the conversation history. Claude's system prompt section in `apps/daemon/src/prompts/system.ts` (Claude only block at the bottom of `composeSystemPrompt`) tells the model to use the tool for 2 to 4 finite choices, and to stop generating tokens after the tool call instead of also writing a markdown duplicate. `AssistantMessage.suppressAskUserQuestionFallbackText` is the belt and suspenders that hides any trailing markdown text in the same turn.
...
- `AskUserQuestionCard` (in `ToolCard.tsx`) submits the answer as a NORMAL user message via `onSubmitForm(formatted)` (→ `ProjectView.handleSend` → new run). It does NOT use the `tool_result` route (`onAnswerToolUse`): claude-code `-p` auto-denies AskUserQuestion and ignores a stdin tool_result, so the model would never register the answer and would re-ask ("回答后让我重选"). Routing as a user message reliably feeds the model AND records the answer in the conversation history. (`onAnswerToolUse`/`toolUseId` remain on the props but are intentionally unused; `submitChatRunToolResult` / `POST /api/runs/:id/tool-result` stay as a generic stdin writer, not for AskUserQuestion.)
