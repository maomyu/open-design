...
async function runFixture(options: {
  ...
  const resourcesRoot = platformName === "darwin"
    ? join(appOutDir, "WorkBuild.app", "Contents", "Resources")
    : join(appOutDir, "resources");
  const appPath = join(appOutDir, "WorkBuild.app");
  ...
  if (platformName === "darwin" && options.writeMacCodeBundleFixture) {
    ...
    await mkdir(join(frameworksRoot, "WorkBuild Helper.app"), { recursive: true });
  }
  ...
  try {
    await runWebStandaloneAfterPack({
      ...
      packager: { appInfo: { productFilename: "WorkBuild" } },
    });
  } catch (error) {
    ...
    try {
      ...
      expect(signedTargets).toEqual(
        expect.arrayContaining([
          ...
          expect.stringMatching(/WorkBuild Helper\.app$/),
          expect.stringMatching(/WorkBuild\.app$/),
        ]),
      );
